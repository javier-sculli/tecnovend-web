        ...
        requestBody: {
          content: json({
            ...
            properties: {
              ...
              reason: { type: 'string', description: 'opcional: motivo por el cual la máquina reporta fuera de servicio o error (ej: sale_timeout, startup, out_of_service, recovered)' },
              ...
              raw_inhibit: { type: 'string', enum: ['service', 'out_of_service'], description: 'opcional: lectura cruda del pin INHIBIT, sin el debounce que aplica in_service' },
              reset_reason: { type: 'integer', description: 'opcional: código crudo de esp_reset_reason() del último reinicio del ESP32' },
              reset_reason_text: { type: 'string', enum: ['poweron', 'external', 'software', 'panic', 'interrupt_wdt', 'task_wdt', 'watchdog', 'deepsleep', 'brownout', 'sdio', 'unknown'], description: 'opcional: motivo del último reinicio, en texto. Viaja en todos los heartbeats; la web solo lo resalta en el heartbeat de arranque (reason=startup)' },
            },
          }),
        },
