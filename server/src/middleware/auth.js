...
export function roleInOrg(userId, clientId) {
  const m = db.prepare('SELECT role FROM memberships WHERE user_id = ? AND client_id = ?').get(userId, clientId);
  return m?.role || null;
}
...
export function requireOrgMember(getClientId) {
  return (req, res, next) => {
    ...
    const role = roleInOrg(req.user.id, clientId);
